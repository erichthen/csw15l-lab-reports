...
```
#### The use of grep -w also prints the lines that match a certain sequence. However, this option will only print the lines where the sequence of characters are stand-alone, i.e. not a substring of another sequence also containing the match. This is extremely helpful in my above example because a normal grep call on "atta" will return all the lines with that sequence, even if it is a substring of another word, like "attack", both being common in this text file.  
```
{
...
```
#### This grep -c command will count the number of lines that contain "a". Since I am combining it with the -w option above, I am looking for the frequency of lines that contain "a" as a word by itself. Using this command shows me that 32 lines contain a lowercase, independent "a" in this file. This is helpful to use once again if you want to get an idea of how frequently the sequence appears in the file.  
